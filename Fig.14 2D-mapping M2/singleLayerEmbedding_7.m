function [Iw, nBit, MSE] = singleLayerEmbedding_7(I,Payload, pFor, IX, IY, IXpos, IYpos,Thresh, noiseLevel)
Iw = I;
...
%         dis = dis + 7/3;
   ...
   %
    ...
    %|x| = 1.5, |y| = 2.5
    if(abs(IX(iP))==1.5 && abs(IY(iP))==2.5)
        bit = mod(data(fix(iP/255)*500+mod(iP,255)),2);
        nData = nData + 1;
        nBit = nBit+1;
        dis = dis + 3;
   ...
   %|x| = 1.5
   if(abs(IX(iP))==2.5 && abs(IY(iP))==2.5)
         bit = mod(data(fix(iP/255)*500+mod(iP,255)),2);
         nData = nData + 1;
         nBit = nBit+1;
         if(bit == 0)
